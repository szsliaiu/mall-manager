                        ...
                        <table class="table table-striped table-bordered table-hover dataTables-example">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                    	<a class="btn btn-primary btn-xs" href="<%=request.getServletContext().getContextPath()%>/admin/goods/getGood/${v.id}">修改</a>&nbsp;&nbsp;
                                    	<a class="btn btn-warning btn-xs" url="<%=request.getServletContext().getContextPath()%>/admin/deleteAdmin/${v.id}" onclick="toDelete(this);" >删除</a>
                                   		<c:if test="${v.status == 0 }">
                                   			<a class="btn btn-warning btn-xs" url="<%=request.getServletContext().getContextPath()%>/admin/deleteAdmin/${v.id}" onclick="toUp(this);" >上架</a>
                                   				<!-- update goods set status = 1 where id = ? -->
                                   		</c:if>
                                   		<c:if test="${v.status == 1 }">
                                   			<a class="btn btn-warning btn-xs" url="<%=request.getServletContext().getContextPath()%>/admin/deleteAdmin/${v.id}" onclick="toDown(this);" >下架</a>
                                   				<!-- update goods set status = 0 where id = ? -->
                                   		</c:if>
                                   		<a class="btn btn-primary btn-xs" href="<%=request.getServletContext().getContextPath()%>/admin/goods/getImg/${v.id}">缩略图</a>&nbsp;&nbsp;                                 		
                                    </td>
                                </tr>
                                </c:forEach>
                            </tbody>
